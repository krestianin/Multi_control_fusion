...
MAX_IMAGES = 10_000  # set to None to use all
...
def main():
    ...
    with open(OUT_CSV, "w", newline="", encoding="utf-8") as f:
        ...
        for filename, cap_list in sorted(captions.items()):
            ...
            selected = [c for _, c in cap_list[:CAPTIONS_TO_USE]]
            caption = " ".join(selected)
            writer.writerow([str(img_path.resolve()), caption])
            written += 1

    print(f"[DONE] Wrote {written} rows to {OUT_CSV} ({skipped} images skipped)")
